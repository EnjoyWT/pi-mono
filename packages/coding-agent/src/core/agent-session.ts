...
export class AgentSession {
	...
	private async _processAgentEvent(event: AgentEvent): Promise<void> {
		if (event.type === "queue_consumed") {
			...
			const fallbackQueueIndex =
				queueIndex === -1 && queuedUserText ? queue.findIndex((message) => message.text === queuedUserText) : -1;
			if (queueIndex !== -1) {
		...
		// Reset overflow recovery as soon as the runtime starts consuming a user message.
	...
	private async _queueSteer(
		...
		const queuedItem = this.agent.steer({
			role: "user",
			content,
			timestamp: Date.now(),
			options: getUserMessageOptions(options),
	...
	private async _queueFollowUp(
		...
		const queuedItem = this.agent.followUp({
			role: "user",
			content,
			timestamp: Date.now(),
			options: getUserMessageOptions(options),
